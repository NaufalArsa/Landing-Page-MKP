...
import { gratificationReportsTable } from "@workspace/db";
...
router.get("/", async (_req, res) => {
  try {
    const reports = await db.select().from(gratificationReportsTable).orderBy(desc(gratificationReportsTable.year));
    res.json(reports);
...
router.get("/:id", async (req, res) => {
  try {
    ...
    const [item] = await db.select().from(gratificationReportsTable).where(eq(gratificationReportsTable.id, id));
...
router.post("/", requireAuth,
  ...
  async (req, res) => {
    try {
      ...
      const { year, title, description } = req.body as { year: string; title: string; description?: string };
      if (!year || !title) { res.status(400).json({ error: "Tahun dan judul wajib diisi" }); return; }
      const [created] = await db.insert(gratificationReportsTable).values({
        year: parseInt(year), title,
...
router.put("/:id", requireAuth,
  ...
  async (req, res) => {
    try {
      ...
      const coverFile = files["cover"]?.[0];
      const { year, title, description } = req.body as { year?: string; title?: string; description?: string };
      const [updated] = await db.update(gratificationReportsTable).set({
        ...
        updatedAt: new Date(),
      }).where(eq(gratificationReportsTable.id, id)).returning();
      res.json(updated);
...
router.delete("/:id", requireAuth, async (req, res) => {
  try {
    ...
    await db.delete(gratificationReportsTable).where(eq(gratificationReportsTable.id, id));

...
import {
  BookOpen, Plus, Trash2, Pencil, Download,
...
export default function AnnualReportList() {
  ...
  const load = () => {
    setLoading(true);
    fetch("/api/annual-reports", { credentials: "include" })
      .then((r) => r.json())
  ...
  const handleDelete = async (id: number) => {
    ...
    setDeletingId(id);
    await fetch(`/api/annual-reports/${id}`, { method: "DELETE", credentials: "include" });
    setDeletingId(null);
    load();
  };
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Kelola laporan tahunan PT. Mitra Karya Prima</p>
          </div>
          <Link href="/admin/laporan-tahunan/baru">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-gray-400 bg-gray-50 rounded-xl border border-dashed border-gray-200">
            ...
            <p className="font-medium text-gray-500">Belum ada laporan tahunan</p>
            <Link href="/admin/laporan-tahunan/baru">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {reports.map((r) => (
              <div key={r.id} className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                {/* Cover */}
                <div className="relative h-48 bg-gradient-to-br from-[#0A6F85] to-[#01B1D7] flex items-center justify-center">
                  {r.coverImageUrl ? (
                    <img src={r.coverImageUrl} alt="" className="w-full h-full object-cover" />
                  ) : (
                    <div className="flex flex-col items-center text-white">
                      <BookOpen className="w-12 h-12 mb-2 opacity-60" />
                      <span className="text-4xl font-extrabold">{r.year}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="p-4">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div>
                      ...
                      <h3 className="font-semibold text-gray-800 text-sm leading-snug line-clamp-2">{r.title}</h3>
                    </div>
                  </div>
                  {r.description && (
                  ...
                  <div className="flex items-center gap-2 pt-3 border-t border-gray-100">
                    <a
                      ...
                      className="flex items-center gap-1 text-xs text-gray-600 hover:text-[#0A6F85] transition-colors"
                    >
                      <FileText className="w-3.5 h-3.5" />
                      Lihat PDF
                    </a>
                    <div className="ml-auto flex items-center gap-1">
                      <Link href={`/admin/laporan-tahunan/${r.id}`}>

...
import {
  Upload, Image, Loader2, CheckCircle2, AlertCircle, X, ScrollText, ZoomIn,
} from "lucide-react";

export default function GcgAdmin() {
  ...
  useEffect(() => {
    fetch("/api/gcg", { credentials: "include" })
      .then((r) => r.json())
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/gcg", { method: "POST", body: fd, credentials: "include" });
  ...
  return (
    <AdminLayout>
      <div className="max-w-3xl mx-auto space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">GCG</h1>
          <p className="text-sm text-gray-500 mt-1">
            Unggah bagan/gambar GCG PT. Mitra Karya Prima. Gambar baru akan menggantikan yang lama.
          </p>
        </div>
            ...
            >
              ...
              {file ? (
                ...
                <div className="flex flex-col items-center gap-2 text-gray-400">
                  <ScrollText className="w-10 h-10 mb-1" />
        ...
        {!loading && previewSrc && (
          <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
            ...
            >
              <img src={previewSrc} alt="GCG" className="w-full h-auto max-h-[500px] object-contain" />
      ...
      {zoomed && previewSrc && (
        ...
        >
          <img src={previewSrc} alt="GCG" className="max-w-full max-h-full object-contain rounded-xl" />
        </div>
      )}
    </AdminLayout>
  );

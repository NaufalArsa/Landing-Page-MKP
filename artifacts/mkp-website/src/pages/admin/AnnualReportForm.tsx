...
export default function AnnualReportForm() {
  ...
  const [form, setForm] = useState<FormState>({ year: String(new Date().getFullYear()), title: "", description: "" });
  ...
  useEffect(() => {
    if (!isEdit) return;
    fetch(`/api/annual-reports/${params.id}`, { credentials: "include" })
      .then((r) => r.json())
      .then((d) => {
        setForm({ year: String(d.year), title: d.title, description: d.description ?? "" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = isEdit ? `/api/annual-reports/${params.id}` : "/api/annual-reports";
      const method = isEdit ? "PUT" : "POST";
      const res = await fetch(url, { method, body: fd, credentials: "include" });
      const data = await res.json();
      if (!res.ok) { setError(data.error ?? "Terjadi kesalahan"); return; }
      setSuccess(true);
      setTimeout(() => navigate("/admin/laporan-tahunan"), 1200);
    } catch {
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-3">
          <Button
            variant="ghost"
            size="sm"
            onClick={() => navigate("/admin/laporan-tahunan")}
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">
            {isEdit ? "Edit Laporan Tahunan" : "Tambah Laporan Tahunan"}
          </h1>
          <p className="text-sm text-gray-500 mt-1">
            {isEdit ? "Perbarui file atau info laporan" : "Unggah PDF dan gambar cover laporan"}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
          ...
          <div>
            ...
            {isEdit && existingPdf && !pdfFile && (
              <p className="text-xs text-gray-500 mt-1.5 flex items-center gap-1">
                ...
                File saat ini:{" "}
                <a href={existingPdf} target="_blank" rel="noopener noreferrer" className="text-[#0A6F85] hover:underline">
                  Lihat PDF
                </a>
              </p>
            )}
          ...
          <div>
            <FileDrop
              ...
              hint="JPG, PNG, atau WebP — tampil sebagai cover buku"
            />
          ...
          {(form.title || form.year) && (
            <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
              ...
              <div className="flex gap-4 items-start">
                <div
                  className="flex-shrink-0 rounded-lg overflow-hidden shadow-md bg-gradient-to-br from-[#0A6F85] to-[#01B1D7] flex flex-col items-center justify-between p-4"
                ...
                >
                  {coverFile ? (
                    ...
                    <>
                      ...
                      <span className="text-white font-extrabold text-xl leading-none">{form.year}</span>
          ...
          <div className="flex justify-end gap-3 pt-2">
            <Button type="button" variant="outline" onClick={() => navigate("/admin/laporan-tahunan")}>
              Batal
            </Button>
            <Button
              type="submit"
            ...
            >
              {submitting ? (
                ...
                <><Upload className="w-4 h-4 mr-2" />{isEdit ? "Simpan Perubahan" : "Simpan Laporan"}</>
              )}
            </Button>
          </div>
        </form>

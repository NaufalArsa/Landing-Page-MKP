...
export default function DewanDireksi() {
  const [members, setMembers] = useState<BoardMember[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("/api/board-members?type=direksi")
      .then((r) => r.json())
        ...
        <div className="container mx-auto px-6 relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <div className="flex items-center gap-2 text-accent/80 text-sm font-medium mb-3">
              ...
              <span className="text-white">Dewan Direksi</span>
            </div>
            <h1 className="text-3xl md:text-5xl font-extrabold text-white">Dewan Direksi</h1>
        ...
        <div className="container mx-auto px-4 md:px-6">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 text-gray-400">
              ...
              <p className="text-lg font-medium">Data Dewan Direksi belum tersedia</p>
